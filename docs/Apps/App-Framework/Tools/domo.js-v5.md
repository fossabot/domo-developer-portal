...
**Custom Implementation Example (v5 ASK-ACK-REPLY Pattern):**

If you're implementing custom event handling in v5, you need to send an acknowledgement using the MessageChannel port:

```javascript
// Custom event handler example (not needed when using Domo.onDataUpdated)
window.addEventListener('message', (event) => {
  if (event.data.type === 'dataUpdate') {
    // Send acknowledgement using the port from the ASK-ACK-REPLY pattern
    ...
    console.log('Dataset updated:', event.data.alias);
  }
});
```

**Note:** This manual acknowledgement is only necessary for custom implementations. When using the standard `Domo.onDataUpdated()`, `Domo.onFiltersUpdated()`, and `Domo.onVariablesUpdated()` methods, acknowledgement is handled automatically.
